...
#
# Requires the full stack from ./run.sh (container + API server). The suite
# is differential: it compares the
...
if ! curl -sf http://127.0.0.1:8081/health >/dev/null; then
  echo "ERROR: API not reachable on 127.0.0.1:8081. Run ./run.sh first." >&2
  exit 1
fi
